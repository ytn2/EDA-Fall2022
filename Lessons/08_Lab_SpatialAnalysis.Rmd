...
huc8_sf.dataframe <- huc8_sf %>% 
  filter(SUBBASIN == "Upper Neuse")


#Create a map
mapview(huc8_sf, col.region="orange", alpha.regions=1) + 
  mapview(huc8_sf.dataframe, col.region="purple", alpha.regions=1)
...
wind_energy <- read.csv("https://raw.githubusercontent.com/ENV859/EnviroAtlasData/main/Wind_Energy.csv", col.classes = c("HUC_12" = "factor")) %>% 
  mutate(huc8_sf = substr(HUC_12,))
