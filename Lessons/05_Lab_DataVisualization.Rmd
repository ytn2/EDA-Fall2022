...
PeterPaulplot.Ex1 <- ggplot(subset(PeterPaul.chem.nutrients, depth = 0), aes(x = daynum, y = temperature_C)) + geom_point(aes(color = year4)) + facet_wrap(vars(lakename), nrow = 2) + ylab("Surface Temperature")
...
PeterPaulplot.Ex2 <- 
  ggplot(PeterPaul.chem.nutrients, aes(x = sampledate, y = temperature_C)) + geom_point(aes(color = depth), size = 0.5) + ylab("Temperature in Celcius") + scale_color_viridis(option = "magma", direction = -1) + scale_x_date(date_breaks = "5 years", date_labels = "%Y")
